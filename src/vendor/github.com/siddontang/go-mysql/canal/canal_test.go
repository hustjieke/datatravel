...
func TestXAPosUpdate(t *testing.T) {
	cfg := NewDefaultConfig()
	cfg.Addr = fmt.Sprintf("%s:3306", *testHost)
	cfg.User = "root"
	cfg.HeartbeatPeriod = 200 * time.Millisecond
	...
	canal, _ := NewCanal(cfg)
	canal.Execute("DROP TABLE IF EXISTS test.canal_test")
	sql := `
        CREATE TABLE IF NOT EXISTS test.canal_test (
			id int AUTO_INCREMENT,
			content blob DEFAULT NULL,
	...
	time.Sleep(time.Second)
	canal.Execute("xa start 'x1';")
	canal.Execute("INSERT INTO test.canal_test (content, name) VALUES ('a1', 'b1')")
	canal.Execute("xa end 'x1';")
	canal.Execute("xa prepare 'x1';")
	canal.Execute("xa commit 'x1';")
	time.Sleep(time.Second)
	pos1 := canal.master.Position()
	t.Logf("pos1:%+v", pos1)

	canal.Execute("xa start 'x1';")
	canal.Execute("INSERT INTO test.canal_test (content, name) VALUES ('a2', 'b22')")
	canal.Execute("xa end 'x1';")
	canal.Execute("xa prepare 'x1';")
	canal.Execute("xa commit 'x1';")
	time.Sleep(time.Second)
	pos2 := canal.master.Position()
	t.Logf("pos2:%+v", pos2)

	if pos1.Pos == pos2.Pos {
		t.Fatalf("pos1.Pos:%+v equal pos2.Pos:%+v", pos1.Pos, pos2.Pos)
	}
}
