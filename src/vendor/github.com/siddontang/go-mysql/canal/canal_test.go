...
func (s *canalTestSuite) TestPosUpdate(c *C) {
	<-s.c.WaitDumpDone()

	s.execute(c, "xa start 'x1';")
	s.execute(c, "INSERT INTO test.canal_test (name,id) VALUES ('a1',13)")
	s.execute(c, "xa end 'x1';")
	s.execute(c, "xa prepare 'x1';")
	s.execute(c, "xa commit 'x1';")

	pos1 := s.c.master.Position()

	s.execute(c, "xa start 'x1';")
	s.execute(c, "INSERT INTO test.canal_test (name,id) VALUES ('a2',15)")
	s.execute(c, "xa end 'x1';")
	s.execute(c, "xa prepare 'x1';")
	s.execute(c, "xa commit 'x1';")

	pos2 := s.c.master.Position()

	c.Assert(pos1.Pos, Not(Equals), pos2.Pos)
}
